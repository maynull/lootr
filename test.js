...
function getStuffed() {
  ...
  loot
    .branch("/equipment/weapons")
    .add({ name: "Uzi" })
    .add({ name: "Pistol" });

  loot
    .branch("/equipment/armor")
    .add({ name: "Plates" })
    .add({ name: "Leather" });
...
/*exports["rollin usage"] = function(test) {
  ...
  test.ok(
    loot.roll("/equipment").name === "Stuff",
    "Should loot a useless equipment"
  );

  test.ok(
    all.indexOf(loot.roll("/equipment", 3, 100).name) > -1,
    "Should loot any equipment"
  );

  test.ok(
    all.indexOf(loot.roll("/equipment", Infinity, Infinity).name) > -1,
    "Should loot any equipment"
  );

  test.ok(
    weapons.indexOf(loot.roll("/equipment/weapons", 3).name) > -1,
    "Should loot a weapon"
  );

  test.ok(
    simplarmors.indexOf(loot.roll("/equipment/armor").name) > -1,
    "Should loot a simple armor"
  );

  test.ok(
    []
      .concat(simplarmors, tougharmors)
      .indexOf(loot.roll("/equipment/armor", 1).name) > -1,
